      ...
      withCustomSparkSession(_.enableHiveSupport) { implicit spark =>
        withLineageTracking { captor =>
          ...
          for {
            ...
            plan1.operations.write.outputSource should equalToUri(s"${warehouseUri.toString}/sourcetable")
            plan2.operations.reads.get.head.inputSources.head should equalToUri(plan1.operations.write.outputSource)
            plan2.operations.write.outputSource should equalToUri(s"${warehouseUri.toString}/targettable")
          }
        }
      }
    }
      ...
      withCustomSparkSession(_.enableHiveSupport) { implicit spark =>
        withLineageTracking { captor =>
          ...
          for {
            ...
            plan.operations.reads.get.head.inputSources.head should equalToUri(s"${warehouseUri.toString}/sourcetable")
