    ...
    withRestartingSparkContext {
      withCustomSparkSession(_.enableHiveSupport()) { spark =>
        withLineageTracking(spark)(lineageCaptor => {
          ...
          plan.operations.write.outputSource should include("path_archive")
          plan.operations.write.append should be(false)
          val writeTable = plan.operations.write.params.get("table").asInstanceOf[Map[String,_]]("identifier").asInstanceOf[TableIdentifier]
          val readTable = plan.operations.reads.get.head.params.get("table").asInstanceOf[Map[String,_]]("identifier").asInstanceOf[TableIdentifier]
