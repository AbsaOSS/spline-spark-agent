...
import za.co.absa.spline.harvester.postprocessing.{AttributeReorderingFilter, OneRowRelationFilter, PostProcessingFilter, ViewAttributeAddingFilter}
...
class DefaultSplineConfigurer(sparkSession: SparkSession, userConfiguration: Configuration)
  ...
  private def internalPostProcessingFilters: Seq[PostProcessingFilter] =
    Seq(new AttributeReorderingFilter(configuration), new OneRowRelationFilter(configuration), new ViewAttributeAddingFilter(configuration))
