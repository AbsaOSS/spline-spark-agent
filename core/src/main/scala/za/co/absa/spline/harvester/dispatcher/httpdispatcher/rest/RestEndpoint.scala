...
class RestEndpoint(val request: HttpRequest, val awsCredentials: Map[String,String], val proxyHost: String, val proxyPort: Int) extends Logging {
  ...
  def head(): HttpResponse[String] = {
    ...
    logInfo(s"HEAD request: ${jsonRequest.toString()}")
  ...
  def post(data: String, contentType: String, enableRequestCompression: Boolean): HttpResponse[String] = {
    ...
    logInfo(s"POST request: ${jsonRequest.toString()}")
