...
import za.co.absa.spline.harvester.SparkLineageInitializer.{InitFlagKey, SparkQueryExecutionListenersKey, SparkStreamingQueryListenersKey}
import za.co.absa.spline.harvester.conf._
import za.co.absa.spline.harvester.listener.QueryExecutionListenerDecorators._
import za.co.absa.spline.harvester.listener.streaming.{SplineStreamingQueryListener, StreamingQueryListenerDelegate}
...
object SparkLineageInitializer {
  ...
  def enableStreamingLineageTracking(sparkSession: SparkSession): SparkSession = {
    sparkSession.enableStreamingLineageTracking()
  }

  def enableStreamingLineageTracking(sparkSession: SparkSession, userConfig: AgentConfig): SparkSession = {
    sparkSession.enableStreamingLineageTracking(userConfig)
  }
  /**
  ...
  implicit class SplineSparkSessionWrapper(sparkSession: SparkSession) {
    ...
    def enableStreamingLineageTracking(userConfig: AgentConfig = AgentConfig.empty): SparkSession = {
      new SparkStreamingLineageInitializer(sparkSession)
        .createListener(isCodelessInit = false, userConfig)
        .foreach(eventHandler =>
          sparkSession.streams.addListener(eventHandler)
        )
      sparkSession
    }
  }
  ...
  val SparkStreamingQueryListenersKey = "spark.sql.streaming.streamingQueryListeners"
}

private[spline] class SparkStreamingLineageInitializer(sparkSession: SparkSession) extends Logging {
  def createListener(isCodelessInit: Boolean, userConfig: AgentConfig = AgentConfig.empty): Option[StreamingQueryListener] = {
  ...
  def createListener(bom: AgentBOM): StreamingQueryListener = {
    ...
    new StreamingQueryListenerDelegate(sparkSession, agent)
  }

  private def withErrorHandling(splineMode: SplineMode)(body: => Option[StreamingQueryListener]) = {
    try {
      body
    } catch {
  ...
  private def assureOneListenerPerSession(body: => StreamingQueryListener) = {
    val isCodelessInitActive =
      (Version.asSimple(spark.SPARK_VERSION) >= ver"2.3.0"
        && sparkSession.sparkContext.getConf
        .getOption(SparkStreamingQueryListenersKey).toSeq
        .flatMap(s => s.split(",").toSeq)
        .contains(classOf[SplineStreamingQueryListener].getCanonicalName))
