...
object ExtraTemplateModel extends Logging{

  class ExtraTemplate(extra: Map[String, Any]) {

    def eval(bindings: Map[String, Any]): Map[String, Any] =
      Try(tryEval(bindings)) match {
        case Success(value) => value
        case Failure(exception) =>
          logWarning("DeclarativeExtraInjectingFilter's template evaluation failed, " +
