...
import com.google.cloud.bigquery.{TableDefinition, TableId}
...
class BigQueryPlugin
  ...
  override def baseRelationProcessor: PartialFunction[(BaseRelation, LogicalRelation), ReadNodeInfo] = {
    case (`_: BigQuerySourceRelation`(bq), _) =>
      val tablePath: String = extractFieldValue[String](bq, "tablePath")
      val defaultTableDefinition: TableDefinition = extractFieldValue[TableDefinition](bq, "defaultTableDefinition")
  ...
  override def relationProviderProcessor: PartialFunction[(AnyRef, SaveIntoDataSourceCommand), WriteNodeInfo] = {
    case (rp, cmd) if rp == "com.google.cloud.spark.bigquery" || BigQuerySourceExtractor.matches(rp) =>
      ...
      (asSourceId(schema, table), cmd.mode, cmd.query, cmd.options)
  }
}
