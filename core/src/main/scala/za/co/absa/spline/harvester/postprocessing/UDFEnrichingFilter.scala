...
class UDFEnrichingFilter extends AbstractPostProcessingFilter {

  override def processExecutionPlan(plan: ExecutionPlan, ctx: HarvestingContext): ExecutionPlan = {
    val fun = plan.expressions.get.functions.get.filter(f =>
      f.extra.get("simpleClassName") == "ScalaUDF" && f.params.get("udfName") == "normalisedCountry")
